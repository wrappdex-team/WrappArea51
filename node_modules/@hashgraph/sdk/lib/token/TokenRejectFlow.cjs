...
class TokenRejectFlow {
  ...
  fillOutTransaction(transaction) {
    ...
    if (this._signPrivateKey) {
      ...
      void transaction.signWith(this._signPublicKey, this._transactionSigner);
    }
  }
  /**
   *
  ...
  async execute(client) {
    const tokenRejectTxn = new _TokenRejectTransaction.default().setTokenIds(this.tokenIds).setNftIds(this.nftIds);
    if (this.ownerId) {
    ...
    const nftTokenIds = this.nftIds.map(nftId => nftId.tokenId).filter(function (value, index, array) {
      return array.indexOf(value) === index;
    });
    const tokenDissociateTxn = new _TokenDissociateTransaction.default().setTokenIds([...this.tokenIds, ...nftTokenIds]);
    ...
    const tokenDissociateResponse = await tokenDissociateTxn.execute(client);
    await tokenDissociateResponse.getReceipt(client);
    return tokenRejectResponse;
  }
  requireNotFrozen() {
    if (this._freezeWithClient != null) {
      throw new Error("Transaction is already frozen and cannot be modified");
    }
  }
}
exports.default = TokenRejectFlow;