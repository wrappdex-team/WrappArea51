...
class FileAppendTransaction extends _Transaction.default {
  ...
  static _fromProtobuf(transactions, signedTransactions, transactionIds, nodeIds, bodies) {
    const body = bodies[0];
    const append = /** @type {HieroProto.proto.IFileAppendTransactionBody} */
    body.fileAppend;
    let contents;
    ...
    for (let i = 0; i < bodies.length; i += incrementValue) {
      const fileAppend = /** @type {HieroProto.proto.IFileAppendTransactionBody} */
      bodies[i].fileAppend;
      ...
      /** @type {Uint8Array} */
      const concat = new Uint8Array(contents.length + /** @type {Uint8Array} */fileAppend.contents.length);
    ...
    const maxChunks = bodies.length ? bodies.length / incrementValue : undefined;
    let chunkInterval;
    if (transactionIds.length > 1) {
      ...
      if (firstValidStart && secondValidStart) {
        chunkInterval = secondValidStart.nanos.sub(firstValidStart.nanos).toNumber();
      }
    }
    return _Transaction.default._fromProtobufTransactions(new FileAppendTransaction({
      fileId: append.fileID != null ? _FileId.default._fromProtobuf(/** @type {HieroProto.proto.IFileID} */
  ...
  freezeWith(client) {
    ...
    for (let chunk = 0; chunk < this.getRequiredChunks(); chunk++) {
      ...
      nextTransactionId = new _TransactionId.default(/** @type {AccountId} */nextTransactionId.accountId, new _Timestamp.default(/** @type {Timestamp} */nextTransactionId.validStart.seconds, /** @type {Timestamp} */nextTransactionId.validStart.nanos.add(this._chunkInterval)));
    }
    this._transactionIds.advance();
  ...
  async executeAll(client, requestTimeout) {
    if (this.maxChunks && this.getRequiredChunks() > this.maxChunks) {
      throw new Error(`cannot execute \`FileAppendTransaction\` with more than ${this.maxChunks} chunks`);
    }
    if (!super._isFrozen()) {
  ...
  _buildIncompleteTransactions() {
    const dummyAccountId = _AccountId.default.fromString("0.0.0");
    const accountId = this.transactionId?.accountId || dummyAccountId;
    const validStart = this.transactionId?.validStart || _Timestamp.default.fromDate(new Date());
    if (this.maxChunks && this.getRequiredChunks() > this.maxChunks) {
      throw new Error(`cannot build \`FileAppendTransaction\` with more than ${this.maxChunks} chunks`);
    }
    ...
    this._signedTransactions.clear();
    for (let chunk = 0; chunk < this.getRequiredChunks(); chunk++) {
      let nextTransactionId = _TransactionId.default.withValidStart(accountId, validStart.plusNanos(this._chunkInterval * chunk));
      this._transactionIds.push(nextTransactionId);
      this._transactionIds.advance();
      if (this._nodeAccountIds.list.length === 0) {
        ...
        for (const nodeAccountId of this._nodeAccountIds.list) {
          this._transactions.push(this._makeSignedTransaction(nodeAccountId));
        }
      }
    }
    this._transactionIds.advance();
