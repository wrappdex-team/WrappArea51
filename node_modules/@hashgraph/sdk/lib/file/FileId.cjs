...
class FileId {
  ...
  static fromSolidityAddress(address) {
    const [shard, realm, file] = EntityIdHelper.fromSolidityAddress(address);
  ...
  static fromEvmAddress(shard, realm, address) {
    ...
    if (!isLongZero) {
      throw new Error("FileId.fromEvmAddress does not support non-long-zero addresses");
    }
    const [shardLong, realmLong, fileLong] = EntityIdHelper.fromEvmAddress(shard, realm, address);
