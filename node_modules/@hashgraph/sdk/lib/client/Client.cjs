...
class Client {
  ...
  constructor(props) {
     ...
     * @type {Hbar}
     */
    this._defaultMaxQueryPayment = new _Hbar.default(1);
    if (props != null) {
  ...
  setDefaultMaxQueryPayment(defaultMaxQueryPayment) {
    const isMaxQueryPaymentNegative = (0, _util.convertToNumber)(defaultMaxQueryPayment.toTinybars()) < 0;
  ...
  async updateNetwork() {
    ...
    try {
      const addressBook = await new _AddressBookQuery.default().setFileId(_FileId.default.getAddressBookFileIdFor(this._shard, this._realm)).execute(this);
      this.setNetworkFromAddressBook(addressBook);
    } catch (error) {
      if (this._logger) {
        this._logger.trace(`failed to update client address book: ${/** @type {Error} */error.toString()}`);
      }
    } finally {
      this._isUpdatingNetwork = false;
    }
  ...
  static _validateNetworkConsistency(network) {
    ...
    const [firstNodeShard, firstNodeRealm] = accountIdStr.split(".").map(Number);
    const isNetworkValid = Object.values(network).every(accountId => {
      const accountIdStr = accountId.toString();
      const [currentShard, currentRealm] = accountIdStr.split(".").map(Number);
      return currentShard === firstNodeShard && currentRealm === firstNodeRealm;
    });
    if (!isNetworkValid) {
      throw new Error("Network is not valid, all nodes must be in the same shard and realm");
    }
  }
