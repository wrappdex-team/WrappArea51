...
declare class Status {
     ...
     */
    valueOf(): HieroProto.proto.ResponseCodeEnum;
}
declare namespace Status {
    ...
    let TokenHasNoMetadataOrSupplyKey: Status;
    let EmptyPendingAirdropIdList: Status;
    let PendingAirdropIdRepeated: Status;
    let MaxPendingAirdropIdExceeded: Status;
    let PendingNftAirdropAlreadyExists: Status;
    let AccountHasPendingAirdrops: Status;
    let ThrottledAtConsensus: Status;
    let InvalidPendingAirdropId: Status;
    let TokenAirdropWithFallbackRoyalty: Status;
    let InvalidTokenInPendingAirdrop: Status;
    let ScheduleExpiryIsBusy: Status;
    let InvalidGrpcCertificateHash: Status;
    let MissingExpiryTime: Status;
    let NoSchedulingAllowedAfterScheduledRecursion: Status;
    let RecursiveSchedulingLimitReached: Status;
    let WaitingForLedgerId: Status;
    let MaxEntriesForFeeExemptKeyListExceeded: Status;
    let FeeExemptKeyListContainsDuplicatedKeys: Status;
    let InvalidKeyInFeeExemptKeyList: Status;
    let InvalidFeeScheduleKey: Status;
    let FeeScheduleKeyCannotBeUpdated: Status;
    let FeeScheduleKeyNotSet: Status;
    let MaxCustomFeeLimitExceeded: Status;
    let NoValidMaxCustomFee: Status;
    let InvalidMaxCustomFees: Status;
    let DuplicateDenominationInMaxCustomFeeList: Status;
    let DuplicateAccountIdInMaxCustomFeeList: Status;
    let MaxCustomFeesIsNotSupported: Status;
    let BatchListEmpty: Status;
    let BatchListContainsDuplicates: Status;
    let BatchTransactionInBlacklist: Status;
    let InnerTransactionFailed: Status;
    let MissingBatchKey: Status;
    let BatchKeySetOnNonInnerTransaction: Status;
    let InvalidBatchKey: Status;
    let ScheduleExpiryNotConfigurable: Status;
    let CreatingSystemEntities: Status;
    let ThrottleGroupLcmOverflow: Status;
    let AirdropContainsMultipleSendersForAToken: Status;
    let GrpcWebProxyNotSupported: Status;
    let NftTransfersOnlyAllowedForNonFungibleUnique: Status;
    let InvalidSerializedTxMessageHashAlgorithm: Status;
    let WrongHookEntityType: Status;
    let EvmHookGasThrottled: Status;
    let HookIdInUse: Status;
    let BadHookRequest: Status;
    let RejectedByAccountAllowanceHook: Status;
    let HookNotFound: Status;
    let LambdaStorageUpdateBytesTooLong: Status;
    let LambdaStorageUpdateBytesMustUseMinimalRepresentation: Status;
    let InvalidHookId: Status;
    let EmptyLambdaStorageUpdate: Status;
    let HookIdRepeatedInCreationDetails: Status;
    let HooksNotEnabled: Status;
    let HookIsNotALambda: Status;
    let HookDeleted: Status;
    let TooManyLambdaStorageUpdates: Status;
    let HookCreationBytesMustUseMinimalRepresentation: Status;
    let HookCreationBytesTooLong: Status;
    let InvalidHookCreationSpec: Status;
    let HookExtensionEmpty: Status;
    let InvalidHookAdminKey: Status;
    let HookDeletionRequiresZeroStorageSlots: Status;
    let CannotSetHooksAndApproval: Status;
    let TransactionRequiresZeroHooks: Status;
    let InvalidHookCall: Status;
    let HooksAreNotSupportedInAirdrops: Status;
    let AccountIsLinkedToANode: Status;
    let HooksExecutionsRequireTopLevelCryptoTransfer: Status;
    let NodeAccountHasZeroBalance: Status;
}
export default Status;
export namespace HieroProto {
    namespace proto {
        type ResponseCodeEnum = import("@hiero-ledger/proto").proto.ResponseCodeEnum;
    }
}
