...
describe('MakerTraits', function() {
    test('allowed sender', function() {
        var traits = _makertraits.MakerTraits.default();
        var sender = _address.Address.fromBigInt(1337n);
        var senderHalf = traits.withAllowedSender(sender).allowedSender();
    ...
    test('nonce', function() {
        ...
        var big_nonce = 1n << 50n;
        expect(function() {
            return traits.withNonce(big_nonce);
        }).toThrow('to big for mask');
    });
    test('expiration', function() {
        var traits = _makertraits.MakerTraits.default();
        var expiration = 1000000n;
    ...
    test('post interaction', function() {
        var traits = _makertraits.MakerTraits.default();
    ...
    test('all', function() {
        var traits = _makertraits.MakerTraits.default().withAllowedSender(_address.Address.fromBigInt(_byteutils.UINT_160_MAX)).allowPartialFills().allowMultipleFills().withEpoch(_byteutils.UINT_40_MAX, _byteutils.UINT_40_MAX).withExpiration(_byteutils.UINT_40_MAX).withExtension().enablePermit2().enableNativeUnwrap().enablePreInteraction().enablePostInteraction();
