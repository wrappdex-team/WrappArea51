 ...
 */ export var FeeTakerExtension = /*#__PURE__*/ function() {
    "use strict";
    ...
    _create_class(FeeTakerExtension, [
        ...
        },
        {
            ...
            value: function build() {
                ...
                var builder = new ExtensionBuilder().withMakingAmountData(this.address, amountGetterData).withTakingAmountData(this.address, amountGetterData).withPostInteraction(new Interaction(this.address, this.buildInteractionData()));
                if (this.makerPermit) {
     ...
     */ function buildAmountGetterData() {
                ...
                var resolverFee = this.fees.resolver.fee.toFraction(Fees.BASE_1E5);
                var builder = new BytesBuilder().addUint16(BigInt(integrator.fee)).addUint8(BigInt(integrator.share)).addUint16(BigInt(resolverFee)).addUint8(BigInt(// contract expects discount numerator, but class contain discount
                Number(Fees.BASE_1E2) - this.fees.resolver.whitelistDiscount.toFraction(Fees.BASE_1E2)));
     ...
     */ function buildInteractionData() {
                var flags = new BN(0n).setBit(FeeTakerExtension.CUSTOM_RECEIVER_FLAG_BIT, Boolean(this.customReceiver));
                var builder = new BytesBuilder().addUint8(flags).addAddress(this.fees.integrator.integrator.toString()).addAddress(this.fees.protocol.toString());
         ...
         */ whitelist, extra) /**
             ...
             */ {
                return new FeeTakerExtension(address, fees, WhitelistHalfAddress.new(whitelist || []), extra === null || extra === void 0 ? void 0 : extra.makerPermit, extra === null || extra === void 0 ? void 0 : extra.extraInteraction, extra === null || extra === void 0 ? void 0 : extra.customReceiver);
            }
        },
        {
            key: "decode",
     ...
     */ function fromExtension(extension) {
                var extensionAddress = Address.fromFirstBytes(extension.makingAmountData);
                assert(Address.fromFirstBytes(extension.takingAmountData).equal(extensionAddress) && Address.fromFirstBytes(extension.postInteraction).equal(extensionAddress), 'Invalid extension, all calls should be to the same address');
                ...
                var customTokensRecipient = flags.getBit(FeeTakerExtension.CUSTOM_RECEIVER_FLAG_BIT) ? new Address(interactionBytes.nextUint160()) : undefined;
                var interactionData = parseAmountData(interactionBytes);
                var extraInteraction = interactionBytes.isEmpty() ? undefined : Interaction.decode(interactionBytes.rest());
                ...
                var permit = extension.hasMakerPermit ? Interaction.decode(extension.makerPermit) : undefined;
                ...
                assert(amountData.whitelist.discount.value === interactionData.whitelist.discount.value, "invalid extension: whitelist discount fee must be same in interaction data and in amount data");
                ...
                assert(interactionData.whitelist.addresses.length === amountData.whitelist.addresses.length, 'whitelist must be same in interaction data and in amount data');
                assert(interactionData.whitelist.addresses.every(function(val, i) {
                    return amountData.whitelist.addresses[i] === val;
                }), 'whitelist must be same in interaction data and in amount data');
                return new FeeTakerExtension(extensionAddress, new Fees(amountData.fees.resolverFee.isZero() ? ResolverFee.ZERO : new ResolverFee(protocolFeeRecipient, amountData.fees.resolverFee, amountData.whitelist.discount), amountData.fees.integratorFee.isZero() ? IntegratorFee.ZERO : new IntegratorFee(integratorFeeRecipient, protocolFeeRecipient, amountData.fees.integratorFee, amountData.fees.integratorShare)), new WhitelistHalfAddress(amountData.whitelist.addresses), permit, extraInteraction, customTokensRecipient);
            }
        }
    ]);
...
function parseAmountData(iter) {
    ...
    var whitelistDiscount = Bps.fromFraction(Number(Fees.BASE_1E2) - Number(iter.nextUint8()), // contract uses 1 - discount
    Fees.BASE_1E2);
