...
export function trim0x(hexString) {
...
export function parseInteractionForField(offsets, interactions, field) {
    const { fromByte, toByte } = getOffsetForInteraction(offsets, field);
    return '0x' + trim0x(interactions).slice(fromByte * 2, toByte * 2);
}
export function getOffsetForInteraction(offsets, field) {
...
export function getMakingAmountForRFQ(amount) {
    return setN(BigInt(amount), 255, true).toString();
}
export function setN(value, bitNumber, flag) {
    const bit = flag ? 1 : 0;
    return value | (BigInt(bit) << BigInt(bitNumber));
}
export function getN(value, n) {
...
export function unpackTimestampAndNoncePredicate(calldata, isSeriesNonceManager = null) {
...
export function packSkipPermitAndThresholdAmount(thresholdAmount, skipPermit) {
...
export function extractWeb3OriginalErrorData(error) {
...
export function compactSignature(signature) {
