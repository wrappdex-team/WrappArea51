...
export function mocksForV3Chain(chainId, contractAddressOverride, seriesNonceManagerContractAddressOverride, domainSettings = { domainName: 'Limit Order Protocol', version: '4' }) {
    const web3Provider = new Web3.providers.HttpProvider(rpcUrls[chainId], { headers: [{ name: 'auth-key', value: process.env.AUTHKEY || '' }] });
    ...
    const facade = new LimitOrderProtocolV3Facade(contractAddress, chainId, providerConnector);
    const limitOrderPredicateBuilder = new LimitOrderPredicateV3Builder(facade);
    const limitOrderBuilder = new LimitOrderV3Builder(providerConnector, domainSettings);
    const erc20Facade = new Erc20Facade(providerConnector);
    const seriesNonceManagerFacade = new SeriesNonceManagerFacade(seriesNonceManagerContractAddress, chainId, providerConnector);
