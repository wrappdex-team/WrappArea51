...
describe('Whitelist', function() {
    it('should encode/decode', function() {
        var whitelist = Whitelist.new(now(), [
            {
                address: Address.fromBigInt(1n),
                allowFrom: now() + 10n
            },
            {
                address: Address.fromBigInt(UINT_160_MAX),
                allowFrom: now() + 20n
            }
        ]);
    ...
    it('Should generate correct whitelist', function() {
        ...
        expect(data.canExecuteAt(Address.fromBigInt(1n), start + 10n)).toEqual(true);
        expect(data.canExecuteAt(Address.fromBigInt(1n), start + 9n)).toEqual(false);
        expect(data.canExecuteAt(Address.fromBigInt(3n), start + 10n)).toEqual(true);
