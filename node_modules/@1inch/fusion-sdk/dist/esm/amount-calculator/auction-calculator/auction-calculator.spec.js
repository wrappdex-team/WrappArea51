...
describe('Auction Calculator', function() {
    it('should be created successfully from suffix and salt', function() {
        ...
        var calculator = AuctionCalculator.fromAuctionData(auctionDetails);
    ...
    describe('Gas bump', function() {
        ...
        var duration = 1800n;
        // 30m
        var takingAmount = parseEther('1');
        var calculator = new AuctionCalculator(now - 60n, duration, 1000000n, [
            {
                delay: 60,
                coefficient: 500000
            }
        ...
        it('0 gwei = no gas fee', function() {
            ...
            expect(calculator.calcAuctionTakingAmount(takingAmount, bump)).toBe(parseEther('1.05'));
        });
        it('0.1 gwei = 0.01% gas fee', function() {
            var bump = calculator.calcRateBump(now, parseUnits('1', 8));
            expect(calculator.calcAuctionTakingAmount(takingAmount, bump)).toBe(parseEther('1.0499'));
        });
        it('15 gwei = 1.5% gas fee', function() {
            var bump = calculator.calcRateBump(now, parseUnits('15', 9));
            expect(calculator.calcAuctionTakingAmount(takingAmount, bump)).toBe(parseEther('1.035'));
        });
        it('100 gwei = 10% gas fee, should be capped with takingAmount', function() {
            var bump = calculator.calcRateBump(now, parseUnits('100', 9));
            expect(calculator.calcAuctionTakingAmount(takingAmount, bump)).toBe(parseEther('1'));
        });
    });
});
