...
describe('Fusion Order', function() {
    it('should create fusion order', function() {
        ...
        var makerTraits = new MakerTraits(BigInt(builtOrder.makerTraits));
    ...
    it('should create fusion order with fees', function() {
        ...
        var order = FusionOrder.new(extensionContract, {
            ...
            fees: Fees.integratorFee(new IntegratorFee(Address.fromBigInt(1n), Address.fromBigInt(2n), Bps.fromPercent(1), Bps.fromPercent(50)))
        });
        var builtOrder = order.build();
        expect(builtOrder).toStrictEqual({
            maker: '0x00000000219ab540356cbb839cbe05303d7705fa',
    ...
    it('should decode fusion order from order + extension', function() {
        ...
        expect(FusionOrder.fromDataAndExtension(order.build(), order.extension)).toStrictEqual(order);
    });
    ...
    it('Should calculate taking amount', function() {
        var now = 10000n;
        var order = FusionOrder.new(new Address('0x8273f37417da37c4a6c3995e82cf442f87a25d9c'), {
        ...
        expect(order.calcTakingAmount(Address.fromBigInt(1n), order.makingAmount, now)).toEqual(2n * order.takingAmount);
    });
    // because init rate bump is 100%
    it('Should calculate fees', function() {
        var order = FusionOrder.fromDataAndExtension({
        ...
        var userAmount = order.getAmountCalculator().getUserTakingAmount(Address.ZERO_ADDRESS, order.makingAmount, order.takingAmount, order.makingAmount, 1738650311n, 1533984564n);
        var integratorFee = order.getAmountCalculator().getIntegratorFee(Address.ZERO_ADDRESS, order.takingAmount, 1738650311n, 1533984564n);
        var protocolFee = order.getAmountCalculator().getProtocolFee(Address.ZERO_ADDRESS, order.takingAmount, 1738650311n, 1533984564n);
    ...
    it('should calculate surplus fee - no surplus', function() {
        var currentTime = now();
        var takerAddress = Address.fromBigInt(1000n);
        var order = FusionOrder.new(Address.ZERO_ADDRESS, {
        ...
        }, // will be 200 at time of fill because of rate bump
        {
            auction: new AuctionDetails({
                ...
                points: [],
                initialRateBump: Number(AuctionCalculator.RATE_BUMP_DENOMINATOR)
            }),
            whitelist: Whitelist.new(0n, [
                {
            ...
            surplus: new SurplusParams(parseUnits('250', 6), Bps.fromPercent(50))
        }, {
            fees: new Fees(new ResolverFee(Address.fromBigInt(123n), Bps.fromPercent(1)), new IntegratorFee(Address.fromBigInt(123n), Address.fromBigInt(123n), Bps.fromPercent(0.1), Bps.fromPercent(10)))
        });
        var makingAmount = parseEther('0.1') / 2n;
    ...
    it('should calculate surplus fee - have surplus', function() {
        ...
        }, // will be 200 at time of fill because of rate bump
        {
            ...
            surplus: new SurplusParams(parseUnits('100', 6), Bps.fromPercent(50))
        }, {
            fees: new Fees(new ResolverFee(Address.fromBigInt(123n), Bps.fromPercent(1)), new IntegratorFee(Address.fromBigInt(123n), Address.fromBigInt(123n), Bps.fromPercent(0.1), Bps.fromPercent(10)))
        });
        var makingAmount = parseEther('0.1') / 2n;
