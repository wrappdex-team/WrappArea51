import { Event, GetAllowMethodsRpcEvent, LimitOrderV4Struct, OrderFilledEvent, OrderFilledPartiallyEvent, OrderInvalidEvent, PingRpcEvent } from '@1inch/fusion-sdk';
...
export type OrderEventType = OrderCreatedEvent | OrderInvalidEvent | OrderBalanceChangeEvent | OrderAllowanceChangeEvent | OrderFilledEvent | OrderFilledPartiallyEvent | OrderCancelledEvent | OrderSecretSharedEvent;
...
export type OrderCreatedEvent = Event<EventType.OrderCreated, {
...
export type OrderBalanceChangeEvent = Event<EventType.OrderBalanceChange, {
    orderHash: string;
    remainingMakerAmount: string;
    balance: string;
}>;
export type OrderAllowanceChangeEvent = Event<EventType.OrderAllowanceChange, {
    orderHash: string;
    remainingMakerAmount: string;
    allowance: string;
}>;
export type OrderCancelledEvent = Event<EventType.OrderCancelled, {
    orderHash: string;
    remainingMakerAmount: string;
}>;
export type OrderSecretSharedEvent = Event<EventType.OrderSecretShared, {
    idx: number;
    secret: string;
...
export type OnOrderNotEnoughAllowanceCb = (data: OrderAllowanceChangeEvent) => any;
export type RpcEventType = PingRpcEvent | GetAllowMethodsRpcEvent | GetActiveOrdersRpcEvent | GetSecretsRpcEvent;
...
export type GetSecretsRpcEvent = RpcEvent<RpcMethod.GetSecrets, SerializableTo<ResolverDataOutput> | {
    error: string;
}>;
