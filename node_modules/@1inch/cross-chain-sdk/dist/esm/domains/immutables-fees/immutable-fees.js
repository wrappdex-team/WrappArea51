...
export class ImmutableFees {
    ...
    static ZERO = new ImmutableFees(0n, 0n, EvmAddress.ZERO, EvmAddress.ZERO);
    static ABI_TYPES = [
        'uint256',
        'uint256',
        'address',
    ...
    constructor(resolverFeeAmount, integratorFeeAmount, resolverFeeRecipient, integratorFeeRecipient) {
        ...
        assert(resolverFeeAmount === 0n || !resolverFeeRecipient.isZero(), 'resolverFeeRecipient must be non-zero when resolverFeeAmount is non-zero');
        assert(integratorFeeAmount === 0n || !integratorFeeRecipient.isZero(), 'integratorFeeRecipient must be non-zero when integratorFeeAmount is non-zero');
    }
    static fromJSON(data) {
        return new ImmutableFees(BigInt(data.resolverFeeAmount), BigInt(data.integratorFeeAmount), EvmAddress.fromString(data.resolverFeeRecipient), EvmAddress.fromString(data.integratorFeeRecipient));
    }
    static decode(bytes) {
        if (bytes === ZX) {
            return ImmutableFees.ZERO;
        }
        const [resolverFeeAmount, integratorFeeAmount, resolverFeeRecipient, integratorFeeRecipient] = coder.decode(ImmutableFees.ABI_TYPES, bytes);
        return new ImmutableFees(BigInt(resolverFeeAmount), BigInt(integratorFeeAmount), EvmAddress.fromString(resolverFeeRecipient), EvmAddress.fromString(integratorFeeRecipient));
    }
    /**
