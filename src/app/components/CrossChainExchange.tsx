...
function buildExchangeRedirectUrl(opts: {
  from: string;
  to: string;
  amount: string;
  address?: string;
...
export function CrossChainExchange() {
  ...
  const handleExchange = useCallback(() => {
    const url = buildExchangeRedirectUrl({
      from: fromToken.symbol,
      to: toToken.symbol,
      amount: amount || "0.1",
      address: toToken.symbol === "HBAR" ? (recipientAddress || walletAddress || undefined) : undefined,
    });
